...
def save_weather_history(weather_entry):
    ...
    db.session.add(weather_record)  # Add the record to the session
...
def home():
    if request.method == "POST":
        ...
        if weather_data:
            ...
            save_weather_history(weather_entry)  # Save the weather data to the database
...
@app.route("/history")
def history():
    history_data = WeatherSearchHistory.query.all()  # Fetch historical weather data from DB
